    ...
    return (
        <>
            {AttributeInfo.map((attributes, index) => (
                <div className="mb-2 px-4 flex md:flex-row flex-col w-full items-center justify-center" key={`${index}`}>

                    {attributes.map((attribue, index)=>(
                        <h1 className={`basis-1/${attribue.num} mr-4 border-b-2 border-b-[#E9EAEB] min-w-[200px]`} key={`${index}`}>
